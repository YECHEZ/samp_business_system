...
#define FS11INS 1 //система денег на сервере:
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == 8002)
    {
        ...
        if(response)
		{
			...
			CallRemoteFunction("GPSrfun", "iiisifff", 2, 1, playIDbus[playerid], busplayname[playIDbus[playerid]],
			busvw[playIDbus[playerid]], buscordx[playIDbus[playerid]], buscordy[playIDbus[playerid]], buscordz[playIDbus[playerid]]);
			printf("[BusSystem] Игрок %s [%d] продал бизнес %s [ID: %d] за %d $ .", RealName[playerid], playerid, busname[playIDbus[playerid]], playIDbus[playerid], buscost[playIDbus[playerid]]);
